...
TEST_CASE_FIXTURE(SodiumFixture, "after a rekey the stream keeps flowing") {
    ...
    std::vector<std::byte> wireBefore(HEADER_SIZE + before.size() +
                                      crypto::TAG_SIZE);
    REQUIRE(pair.Sealer.Seal(MutableByteSpan{wireBefore}, ByteSpan{before})
                .has_value());
    ...
    std::vector<std::byte> wireAfter(HEADER_SIZE + after.size() +
                                     crypto::TAG_SIZE);
    REQUIRE(pair.Sealer.Seal(MutableByteSpan{wireAfter}, ByteSpan{after})
                .has_value());
    ...
    const auto openedBefore =
        pair.Opener.Open(MutableByteSpan{out}, *frameBefore);
    REQUIRE(openedBefore.has_value());
    CHECK(std::vector<std::byte>(openedBefore->begin(), openedBefore->end()) ==
          before);

    const auto frameAfter = ReadFrame(ByteSpan{wireAfter}, MAX_FRAME);
    REQUIRE(frameAfter.has_value());
    CHECK(frameAfter->Header.Epoch == 1);
    const auto openedAfter =
        pair.Opener.Open(MutableByteSpan{out}, *frameAfter);
    REQUIRE(openedAfter.has_value());
    CHECK(std::vector<std::byte>(openedAfter->begin(), openedAfter->end()) ==
          after);
    CHECK(pair.Opener.Epoch() == 1);
}

TEST_CASE_FIXTURE(SodiumFixture,
                  "frames still in flight from the old epoch are not lost") {
    ...
    REQUIRE(
        pair.Sealer.Seal(MutableByteSpan{wireOld}, ByteSpan{old}).has_value());
    REQUIRE(pair.Sealer.Rekey().has_value());

    std::vector<std::byte> wireFresh(HEADER_SIZE + fresh.size() +
                                     crypto::TAG_SIZE);
    REQUIRE(pair.Sealer.Seal(MutableByteSpan{wireFresh}, ByteSpan{fresh})
                .has_value());
...
TEST_CASE_FIXTURE(SodiumFixture,
                  "an epoch that is neither current nor next is refused") {
    Pair pair;
    ...
    std::vector<std::byte> out(plaintext.size());
    for (std::uint8_t epoch :
         {std::uint8_t{2}, std::uint8_t{7}, std::uint8_t{255}}) {
        FrameView forged = *frame;
...
TEST_CASE_FIXTURE(SodiumFixture,
                  "rekeying gives a different key, not the same one renamed") {
    Pair pair;
    ...
    // but bump a number, this would be a nonce reuse and the bodies would
    // match.
    const auto bodyFirst = ByteSpan{first}.subspan(HEADER_SIZE);
    const auto bodySecond = ByteSpan{second}.subspan(HEADER_SIZE);
    CHECK_FALSE(
        std::equal(bodyFirst.begin(), bodyFirst.end(), bodySecond.begin()));
}

TEST_CASE_FIXTURE(SodiumFixture,
                  "the sender asks for a rekey once its limit is reached") {
    FrameSealer sealer{MakeKey(1), MakeSalt(1), 2};
...
TEST_CASE_FIXTURE(SodiumFixture,
                  "the epoch does not wrap past its last value") {
...
TEST_CASE_FIXTURE(SodiumFixture,
                  "the receiver's limit applies to the new epoch too") {
    FrameSealer sealer{MakeKey(1), MakeSalt(1)};
    FrameOpener opener{MakeKey(1), MakeSalt(1), 1};
...
TEST_CASE_FIXTURE(SodiumFixture,
                  "a receiver at the last epoch does not prepare another") {
...
TEST_CASE_FIXTURE(SodiumFixture,
                  "a bad frame from the next epoch does not redo the ratchet") {
